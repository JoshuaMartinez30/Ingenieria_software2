...
def get_numero_factura():
    ...
    try:
        ...
        # Obtener el SAR con el menor id_sar activo relacionado a esa sucursal
        query_sar = """
        SELECT id_sar
        FROM sar
        WHERE id_sucursal = %s AND estado = 'Activo'
        ORDER BY id_sar ASC
        LIMIT 1
        """
        ...
        # Leer todos los resultados anteriores (si hay más)
        cursor.fetchall()  

        if not sar_record:
            print("No se encontró un SAR válido y activo para la sucursal del usuario.")
            return None
...
def get_ver_pedido_apagado(id_pedido):
    ...
    try:
        ...
        sar_records = cursor.fetchall()  # Obtener todos los registros
...
@app_pedido.route('/guardar_total', methods=['POST'])
def guardar_total():
    data = request.get_json()
    session['total'] = data['total']
    return '', 204  # Devuelve un estado HTTP 204 (sin contenido)
...
def edit_pedido(id_pedido):
    ...
    total = session.get('total')  # Obtener el total de la sesión
...
# Asegúrate de que los datos de clientes, métodos y estados se cargan correctamente
    ...
    now = datetime.now()
    return render_template('edit_pedido.html', pedido=pedido, clientes=clientes, estados=estados, metodos=metodos, total=total, now=now)
