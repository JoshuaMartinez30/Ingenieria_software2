...
def impuesto_exists(tipo_impuesto, tasa_impuesto):
    connection = create_connection()
    if connection is None:
        return False
    cursor = connection.cursor()
    query = "SELECT COUNT(*) FROM impuesto WHERE tipo_impuesto = %s AND tasa_impuesto = %s"
    cursor.execute(query, (tipo_impuesto, tasa_impuesto))
...
def insert_user(tipo_impuesto, tasa_impuesto):
    if impuesto_exists(tipo_impuesto, tasa_impuesto):
        return False  # Retorna False si ya existe el impuesto
...
def update_user(id_impuesto, tipo_impuesto, tasa_impuesto):
    ...
    # Verificar si el impuesto ya existe (excluyendo el que se está actualizando)
    check_query = "SELECT COUNT(*) FROM impuesto WHERE tipo_impuesto = %s AND tasa_impuesto = %s AND id_impuesto != %s"
    cursor.execute(check_query, (tipo_impuesto, tasa_impuesto, id_impuesto))
    exists = cursor.fetchone()[0]

    if exists > 0:
        print("El impuesto ya existe en la base de datos.")
    ...
    try:
        print(f"Ejecutando actualización: {query} con valores {values}")
        cursor.execute(query, values)
        connection.commit()
        print("Impuesto actualizado exitosamente.")
        return True
    except Error as e:
        print(f"The error '{e}' occurred")
        return False
...
def submit():
    ...
    if insert_user(tipo_impuesto, tasa_impuesto):
        ...
        flash('El impuesto ya existe en la base de datos. No se puede guardar un impuesto duplicado.')
