...
@app_sar.route('/sars')
def sars():
    # Obtén los datos de SAR
    sar = get_sar()
    
    # Lógica para la paginación
    page = request.args.get('page', 1, type=int)  # Obtiene el número de página desde la URL
    per_page = request.args.get('per_page', 5, type=int)  # Obtiene cuántos elementos mostrar por página

    # Aquí puedes agregar la lógica de paginación si es necesario
    total_items = len(sar)  # Suponiendo que tienes un conteo total de items
    total_pages = (total_items + per_page - 1) // per_page  # Calcula el total de páginas
    
    # Puedes usar un slice para obtener solo los elementos de la página actual
    start = (page - 1) * per_page
    end = start + per_page
    sar_paginated = sar[start:end]

    return render_template('sars.html', sar=sar_paginated, page=page, total_pages=total_pages, per_page=per_page)
